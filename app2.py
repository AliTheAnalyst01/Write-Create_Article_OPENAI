...
def main() -> None:
    ...
    summary_model = st.sidebar.selectbox(
        "Select Summary Model", options=["gpt-3.5-turbo", "gpt-4"]
    )
    ...
    if write_article:
        ...
        with st.status("Reading News", expanded=False) as status:
            with st.container():
                ...
                for r in results:
                    if "url" in r:
                        # Load article using LangChain
                        loader = UnstructuredURLLoader(urls=[r["url"]])
                        docs = loader.load()
                        news_results.append({"title": r["title"], "date": r["date"], "url": r["url"], "docs": docs})
            ...
            with st.status("Summarizing News", expanded=False) as status:
                with st.container():
                    ...
                    for news_result in news_results:
                        ...
                        # Check if 'docs' contains any content
                        if news_result["docs"]:
                            news_summary += f"#### Introduction\n\n{news_result['docs'][0].page_content}\n\n"
                        else:
                            news_summary += "#### Introduction\n\nNo content available for this article.\n\n"

                        # Use LangChain to generate summary
                        text_splitter = RecursiveCharacterTextSplitter(chunk_size=per_article_summary_length, chunk_overlap=20)
                        documents = text_splitter.split_documents(news_result["docs"])

                        # Summarize each document individually
                        summaries = []
                        llm = OpenAI(temperature=0.7)  # No need for model_name here
                        for doc in documents:
                            summary = llm(doc.page_content)  # Summarize the document content directly
                            summaries.append(summary)

                        # Combine the individual summaries
                        combined_summary = "\n\n".join(summaries)
                        _summary_length = len(combined_summary.split())
                        if _summary_length > news_summary_length:
                            combined_summary = truncate_text(combined_summary, news_summary_length)
        ...
        with st.spinner("Writing Article..."):
            ...
            response = openai.ChatCompletion.create(
                model=summary_model,  # Directly specify the model
                messages=[
                    {"role": "system", "content": "Write the article based on the provided summary."},
